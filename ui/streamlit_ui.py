...
from ui.chat_interface import render_input, render_messages, render_welcome
from ui.sidebar import render_sidebar
from ui.upload_interface import (
...
_STRIP_KEYWORDS = [
    "en pdf", "en word", "en formato pdf", "en formato word",
    "dame pdf", "dame word", "quiero pdf", "quiero word",
    "crea un", "genera un", "escribe un", "escribe una",
    "como pdf", "como word", "en docx", "formato docx",
    "archivo pdf", "archivo word", "exportar pdf", "exportar word",
]
...
def _extract_title(user_input: str) -> str:
    ...
    for kw in _STRIP_KEYWORDS:
        titulo = titulo.replace(kw, "")
    titulo = titulo.strip().capitalize()
    if len(titulo) < 3:
        titulo = "Documento CampusMind AI"
    return titulo
...
def _init_session() -> None:
    ...
    # FIX: almacén persistente de imágenes entre reruns
...
def main() -> None:
    ...
    if cfg["uploaded_images"]:
        ...
        # FIX: process_image_uploads ahora guarda bytes en session_state internamente
    ...
    if user_input:
        ...
        with st.chat_message("assistant", avatar="🎓"):
            with st.spinner("Pensando…"):
                ...
                if doc_fmt:
                    tipo = "PDF" if doc_fmt == "pdf" else "Word"
                    actual_input = (
                        f"{user_input}\n\n"
                        f"INSTRUCCIÓN IMPORTANTE: Genera el documento completo con todo "
                        f"su contenido directamente. Usa formato markdown: "
                        f"# para título principal, ## para subtítulos, "
                        f"### para secciones, **negrita** para énfasis, - para listas. "
                        f"NO digas que no puedes crear archivos {tipo}. "
                        f"NO pongas frases como 'aquí tienes' o 'a continuación'. "
                        f"Escribe ÚNICAMENTE el contenido del documento."
                    )
 
                if cfg["active_agent"]:
                    # FIX: usar image_bytes desde session_state (persiste entre reruns)
            ...
            if doc_fmt:
                ...
                titulo = _extract_title(user_input)
                ...
                st.session_state.pending_download = {
                    ...
                    "filename": f"{titulo[:40].replace(' ', '_')}.{doc_fmt}",
                }
